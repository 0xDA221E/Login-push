...
if [[ $1 != "install" ]]; then
  ...
  if [[ $2 == "telegram" ]]; then
    ...
    botname=$(curl 'https://api.telegram.org/bot'$TELEGRAM_HTTPTOKEN'/getMe' | sed -e 's/[{}]/''/g' | awk -v RS=',"' -F: '/^username/ {print $2}')
    ...
    tgname=$(
      ...
      python3 -c '
      import json,sys;
      obj=json.load(sys.stdin);print(obj["result"][0]["message"]["from"]["first_name"] + " "+  obj["result"][0]["message"]["from"]["last_name"]);
      ')
    ...
    TELEGRAM_USERID=$(
      ...
      python3 -c '
      import json,sys;
      obj=json.load(sys.stdin);print(obj["result"][0]["message"]["from"]["id"]);
      ')
